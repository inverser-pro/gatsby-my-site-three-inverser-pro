---
title: "Видео+код: #3/8 ThreeJS shader, postprocessing, glitch, анимация при движении мышью PMREMGenerator"
short_to_menu: "#3-8 Mando|shader, postprocessing, glitch"
date: 2023-11-13 18:42:08
description: "Видео+код: #3/8"
published: true
category: article
artPre: "/_img/2023/3-8"
---

# Видео: #3/8 ThreeJS shader, postprocessing, glitch, анимация при движении мышью PMREMGenerator

<span className="loadYoutube" data-id="C3vVC6Pp9Ro"></span>

## Файлы из урока 3/8 по Mando

[3-8-shader-glitch.7z](/fls/2023/3-8-shader-glitch.7z "Скачать файл [55 MB]")
...
```
00:00 Вступление
00:23 Что будет в этом уроке?
01:12 Результат урока
01:56 Приступаем к кодингу
03:05 Исправление «гиперпрыжка» — анимаций линий с 0 до полного размера
04:20 geomenty.translate() в ThreeJS
07:14 Перенос участков кода
07:53 Добавление кеширования HDR карты
09:33 GUI — отладка с пом. визуальных инструментов
15:17 texture mapping — с ним и без него
15:34 Фотореалистичность в ThreeJS (нстройки и параметры сцены)
16:42 Добавление зеркала (отражения) для объекта на сцене ThreeJS
19:24 clipBias, multisample в Reflector ThreeJS
22:39 ThreeJS GUI — регулируем toneMapping в слайдере
29:44 Добавление shader'ного материала с вращением карты окружения для 3D модели ThreeJS
36:10 toneMapping — разные настройки тонирования конечного изображения рендеринга сцены
39:05 PMREMGenerator в ThreeJS
40:42 Крутящаяся shader'ная карта окружения для брони и шлема Мандо
41:52 GLSL rotationMatrix
44:22 shader material uniforms
45:00 За счёт чего проихсодит анимация внутри шейдера (GLSL)?
52:24 RGBELoader vs TextureLoader
58:47 Добавим PostProcessing (EffectComposer, UrealBloomPass, ShaderPass, RenderPass)
1:07:34 DotScreenShader ThreeJS — создаём shader Glitch эффект постпроцессинга
1:11:47 Simplex 3D Noise для GLSL и ThreeJS
1:14:33 Добавляем свои переменные в Shader GLSL и JS (uniforms)
1:17:28 Shader GLSL vUv, texture2D
1:22:17 Создаём эффект glithc по размеру окна браузера — передаём данные в GLSL
1:29:19 Добавим цвет заливки всей сцены ThreeJS для эффекта голограммы
1:33:02 Добавим эффект glitch/голограммы при движении мыши
1:36:59 Делаем анимацию Shader GLSL при движении мыши более плавной
1:38:29 Добавим эффект наклона шлема при движении мыши
1:44:26 Результат наших с вами стараний
1:45:38 Попытка устранить дёргание шлема
1:48:32 До новых встреч
```